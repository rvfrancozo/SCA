    ...
    <div class="d-flex gap-2 row m-auto">
        <main class="col-5 border border-black p-0 card">
            <div class="card-header">
                <h3 class="m-0">ADD OPTION TO DA {{ $decision_area_id }}</h3>
            </div>
            <form action="{{ route('option.create', ['project_id' => $project_id, 'decision_area_id' => $decision_area_id]) }}" method="POST" class="card-body d-flex flex-column align-items-center justify-content-between gap-2" style="min-height: 40dvh;">
                @csrf
                @if ($errors->any())
